...
export default function CustomerSignup({ navigation }: Props) {
  ...
  const handleSignup = () => {
    (async () => {
      ...
      try {
        // Standalone frontend: accept registration locally if passwords match.
        await new Promise((r) => setTimeout(r, 150));
        // In this simplified flow we accept any registration and proceed
        navigation.replace('CustomerTabs');
        
      } catch (e: any) {
        console.error('Signup error', e);
        alert('Signup failed: ' + (e.message || 'Error'));
      }
    })();
  };
